...
final class PageContent {
				...
				<section id="cb-profiles-tab-general" class="cb-profiles-tab-panel" role="tabpanel" aria-labelledby="cb-profiles-tab-general-button" data-cb-profiles-panel="general">
					...
					<div class="cb-core-card" data-cb-profiles-requires-enabled>
						...
						<div class="cb-core-card__body">
							...
							<details class="cb-core-disclosure cb-core-disclosure--compact cb-core-disclosure--subtle cb-profiles-role-disclosure" data-cb-profiles-role-disclosure>
								<summary class="cb-core-disclosure__summary">
									<svg class="cb-core-disclosure__icon" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true"><path d="M6 3l5 5-5 5" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"/></svg>
									<span class="cb-core-disclosure__title"><?php esc_html_e( 'Hide profiles for selected roles', 'core-blueprint-profiles' ); ?></span>
									<span class="cb-core-disclosure__meta"><?php esc_html_e( 'Selected roles:', 'core-blueprint-profiles' ); ?> <strong data-cb-profiles-role-count><?php echo esc_html( (string) count( (array) $s['excluded_roles'] ) ); ?></strong></span>
								</summary>
								<div class="cb-core-disclosure__body">
									<p class="description"><?php esc_html_e( 'Users with these roles do not have a profile page; their profile URL returns a real 404.', 'core-blueprint-profiles' ); ?></p>
									<div class="cb-profiles-role-grid">
										<?php foreach ( $roles as $slug => $role ) : ?><label><input type="checkbox" name="<?php echo esc_attr( Settings::OPTION ); ?>[excluded_roles][]" value="<?php echo esc_attr( $slug ); ?>" <?php checked( in_array( $slug, (array) $s['excluded_roles'], true ) ); ?> data-cb-profiles-role-checkbox> <span><?php echo esc_html( translate_user_role( $role['name'] ) ); ?></span> <code><?php echo esc_html( $slug ); ?></code></label><?php endforeach; ?>
									</div>
								</div>
							</details>
						</div>
	...
	private static function checkbox( string $key, string $label, mixed $checked, string $description, bool $enabled_control = false ): void {
		...
		$classes = 'cb-core-field' . ( $enabled_control ? ' cb-core-field--enable' : '' );
		?>
		<div class="<?php echo esc_attr( $classes ); ?>"<?php echo $enabled_control ? ' data-cb-profiles-enabled-control' : ''; ?>><input type="hidden" name="<?php echo esc_attr( $name ); ?>" value="0"><label class="cb-core-field__label"><input type="checkbox" name="<?php echo esc_attr( $name ); ?>" value="1" <?php checked( $checked ); ?>> <?php echo esc_html( $label ); ?></label><p class="description"><?php echo esc_html( $description ); ?></p></div>
		<?php
	}
}
